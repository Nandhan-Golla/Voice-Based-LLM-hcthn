...
def update_system_prompt():
    ...
    if user_memories:
        return f"{base_prompt} I've got your memories locked in: {user_memories}. I’ll weave them into our chats to make things super personal!"
    return base_prompt
...
def text_to_speech(text: str):
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp3', delete=False) as temp_mp3:
            ...
            faster_audio = audio.speedup(playback_speed=SPEECH_SPEED, crossfade=30)
            with tempfile.NamedTemporaryFile(suffix='.mp3', delete=False) as temp_fast_mp3:
                faster_audio.export(temp_fast_mp3.name, format='mp3')
                pygame.mixer.music.load(temp_fast_mp3.name)
                pygame.mixer.music.play()
                while pygame.mixer.music.get_busy():
                    pygame.time.Clock().tick(10)
                os.unlink(temp_fast_mp3.name)
...
def process_memories(file_content: str):
    ...
    try:
        ...
        return "Got your memories loaded, buddy! I'm ready to make this personal."
    except Exception as e:
        return f"Whoops, couldn't read that file: {str(e)}"
